...
__device__ int GPU_FitFunction(const float *x, float *fvec, void *FitFuncPrivDataPtr)
{
#define A x[0]
#define x0 x[1]
#define y0 x[2]
		...
		for(j0=0; j0<GPU_f_Cs->sv_ImgDim; j0++){
			for(i0=0; i0<GPU_f_Cs->sv_ImgDim; i0+=bdim){
				if((i0+tidx)<GPU_f_Cs->sv_ImgDim){
					TwoD_Gaussian[tidx] = B + cuFABS(A)*(cuEXP(-0.5f*(((i0+tidx)-x0)*((i0+tidx)-x0)+
						(j0-y0)*(j0-y0))/s/s));
...
/* Analytical Jacobian */
__device__ int GPU_AnalyticalJacobian(const float *x, float *FJac, void *JacFuncPrivDataPtr)
{
#define A x[1]
#define x0 x[2]
#define y0 x[3]
...
#define CommonItem2 (j0-y0)
	...
	if(GPU_f_Cs->sv_JacMethod != 1) return(-1); // Analytical Jacobian is only avaiable for uWLS.
	...
	case 1: // uWLS
		for(j0=0; j0<GPU_f_Cs->sv_ImgDim; j0++){
			for(i0=0; i0<GPU_f_Cs->sv_ImgDim; i0+=bdim){
				if((i0+tidx)<GPU_f_Cs->sv_ImgDim){	
					CommonItem[tidx] = cuEXP(-0.5f/(s*s)*CommonItem1*CommonItem1
						-0.5f/(s*s)*CommonItem2*CommonItem2);
					FJac[0*GPU_f_Cs->sv_m + (j0*GPU_f_Cs->sv_ImgDim + (i0+tidx))] = 1.0f;
